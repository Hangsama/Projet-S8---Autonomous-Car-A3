...
def detecter(image):
    ...
    #panneau stop, feurouge
    lower_red = np.array([0,100,100])
    upper_red = np.array([20,255,255])
    mask = cv2.inRange(hsv, lower_red, upper_red)
    kernel = np.ones((5,5),np.uint8)
    dilatation = cv2.dilate(mask,kernel,iterations = 4)#panneau
    dilatation2 = cv2.dilate(mask,kernel,iterations = 1)#feurouge
    res = cv2.bitwise_and(image,image, mask= dilatation)
    res2 = cv2.bitwise_and(image,image, mask= dilatation2)#feurouge
    gray = cv2.cvtColor(res, cv2.COLOR_BGR2GRAY)
    gray2 = cv2.cvtColor(res2, cv2.COLOR_BGR2GRAY)#feurouge
    ret, thresh= cv2.threshold(gray,140, 255, cv2.THRESH_BINARY)
    ret2, thresh2= cv2.threshold(gray2,140, 255, cv2.THRESH_BINARY)
    contours,h = cv2.findContours(thresh,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    contours2,h2 = cv2.findContours(thresh,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    for cnt in contours:
    ...
    for cnt2 in contours:
        perimetre2=cv2.arcLength(cnt2,True)
        approx2 = cv2.approxPolyDP(cnt2,0.01*perimetre2,True)
        if len(approx2)>=8 :
            cv2.drawContours(res2,[cnt2],-1,(0,255,0),2)
            for i in range(len(contours2)):
                area2 = cv2.contourArea(contours2[i])
                if area2 > areaRouge:
                    areaRouge = area
    return areaPanneau, areaRouge;
...
def main():
        ...
        GPIO.output(Trig,False)#etat fermer
        p = GPIO.PWM(12, 63)#pin=12, freq=63Hz
        #demarrer moteur
        p.start(9.4)
        time.sleep(2)
        #foward
...
""".format(RESOLUTION_X, RESOLUTION_Y, SCAN_POS_X, SCAN_HEIGHT, SCAN_RADIUS_REG, NUMBER_OF_CIRCLES, SCAN_RADIUS*2)
        ...
        with picamera.PiCamera() as camera:
                ...
                while True:
                        ...
                        commandMoteur(bearing, ser)
                        #stop or forward
                        distance = ultrason()
                        areaPanneau, areaRouge = detecter(image)
                        if distance < 40:
                            p.start(0)#PWM0
                            #print "obstacle"
                        elif areaRouge > 15 and areaRouge < 90:
                            p.start(0)#Feurouge
                            #print "feurouge"
                        elif areaPanneau >600 and areaPanneau< 3000:
                            p.start(0)#panneau stop
                            #print "panneau"
                        else:
                            p.start(9.90)#forward
